...
The default model is `gemini-3.7-flash` for the requested connection test.
Override it without changing code if your account exposes another model:

```bash
GEMINI_MODEL=gemini-3.7-flash uv run agentic-cinema "Write a tense opening scene."
```
...
- The Google Gen AI SDK is used directly so the requested Gemini model can be
  selected explicitly.
- If Google no longer serves `gemini-3.7-flash` for your account, set
