...
- Keep the requested model name `gemini-2.0-flash` as the default for
  now.

## Gotchas

- The configured API account previously rejected the unavailable model during a
  Gemini request. The code now uses `gemini-2.0-flash` and exposes
